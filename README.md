<!-- BANNER -->
<p align="center">
  <img src="https://i.imgur.com/7M1K4nL.png" width="100%" alt="Banner">
</p>

<h1 align="center">👋 Olá, eu sou o Razec (eorazec)</h1>

<p align="center">
  Desenvolvedor Full-Stack • Criador de Bots • Sistemas de Autenticação • Painéis e APIs
</p>

---

## 🚀 Sobre mim

- 🔥 Desenvolvedor focado em **Node.js, APIs REST, WebSockets, bots e sistemas automatizados**  
- 📦 Crio **bibliotecas NPM, bots profissionais, painéis completos, dashboards e auth systems**  
- 🛠️ Experiência com **Discord Bots, gateways, segurança, criptografia, automação, proxies e redes**  
- 🌐 Também trabalho com **front-end**, criando interfaces rápidas e responsivas  
- 🧠 Sempre estudando arquitetura, sistemas distribuídos e devops  

---

## 🛠️ Tecnologias & Competências

### **Linguagens**
<p>
  <img src="https://img.shields.io/badge/JavaScript-000?style=for-the-badge&logo=javascript">
  <img src="https://img.shields.io/badge/TypeScript-000?style=for-the-badge&logo=typescript">
  <img src="https://img.shields.io/badge/Python-000?style=for-the-badge&logo=python">
</p>

### **Back-end**
<p>
  <img src="https://img.shields.io/badge/Node.js-000?style=for-the-badge&logo=node.js">
  <img src="https://img.shields.io/badge/Express-000?style=for-the-badge&logo=express">
  <img src="https://img.shields.io/badge/MySQL-000?style=for-the-badge&logo=mysql">
  <img src="https://img.shields.io/badge/Prisma-000?style=for-the-badge&logo=prisma">
</p>

### **Front-end**
<p>
  <img src="https://img.shields.io/badge/HTML5-000?style=for-the-badge&logo=html5">
  <img src="https://img.shields.io/badge/CSS3-000?style=for-the-badge&logo=css3">
  <img src="https://img.shields.io/badge/TailwindCSS-000?style=for-the-badge&logo=tailwindcss">
  <img src="https://img.shields.io/badge/React-000?style=for-the-badge&logo=react">
</p>

### **DevOps & Ferramentas**
<p>
  <img src="https://img.shields.io/badge/Docker-000?style=for-the-badge&logo=docker">
  <img src="https://img.shields.io/badge/Nginx-000?style=for-the-badge&logo=nginx">
  <img src="https://img.shields.io/badge/Git-000?style=for-the-badge&logo=git">
  <img src="https://img.shields.io/badge/Linux-000?style=for-the-badge&logo=linux">
</p>

---

## 🧩 Projetos em Destaque

### 🔹 **Talkky Bot — Node.js Package**
Biblioteca completa para integração com a plataforma Talkky.

🔗 **Repositório:** *(adicione quando publicar)*  
📦 **NPM:** *(adicione quando publicar)*

---

## 📊 Estatísticas do GitHub

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=eorazec&show_icons=true&theme=tokyonight&count_private=true" />

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=eorazec&layout=compact&theme=tokyonight" />

</div>

---

## 📫 Contato

- 💼 **Portfólio:** *(adicione quando quiser)*
- ✉️ **Email:** *(coloque aqui se quiser)*  
- 🧵 **Discord:** `razec.dev`  
- 🐙 **GitHub:** [github.com/eorazec](https://github.com/eorazec)

---

## ⭐ Apoie

Se gostar dos meus projetos, deixe uma ⭐ nos repositórios!  
Isso ajuda muito e me motiva a continuar criando ferramentas melhores.

---

<p align="center">Feito com 💙 por <strong>eorazec</strong></p>
